...
const EntryScreen = ({ navigation }) => {
  ...
  const [doh, setDoh] = useState('Click here to select the date of the hike');
  ...
  const handleAddHike = async () => {
    if (!title || !location ||!doh ||!checked ||!length ||!currentValue || !description) {
      Alert.alert("Error", "Please enter title and description");
      return;
    }
    await Database.addHike(title, location, doh, checked, length, currentValue, description);
    navigation.goBack();
  };
  ...
  return (
    <View style={styles.container}>
      <View style={{
        ...
        <Text style={{ color: "#fff", fontSize: 24, textAlign: 'center', fontWeight: 'bold', marginTop: 30, fontFamily: 'Knewave'}}>Add Hike</Text>
      </View>
      {/* Name */}
      ...
      <TextInput style={styles.input} value={location} onChangeText={setLocation}></TextInput>
      
      <Text style={styles.text}>Date of the hike</Text>
      <TouchableOpacity style={styles.input} onPress={() => showMode("date")} >
        <Text style={{ color: 'gray' }} >{doh}</Text>
      </TouchableOpacity>
      {
        show && (
          <DateTimePicker
        ...
        <View style={styles.wrapper}>
          {['Yes', 'No'].map(feeling =>
            <View key={feeling} style={styles.mood}>
              ...
              <TouchableOpacity style={styles.outter} onPress={() => setChecked(feeling)}>
                {checked === feeling && <View style={styles.inner} />}
              </TouchableOpacity>
      ...
      <View style={styles.lengthParkBg}>
        ...
        <TextInput style={styles.inputSmall} placeholder='Km' keyboardType="numeric"
          value={length} onChangeText={setLength}></TextInput>
      </View>
      ...
      <View style={styles.lengthParkBg}>
        ...
        <View>
          ...
          }} items={items} open={isOpen} setOpen={() => setIsOpen(!isOpen)} value={currentValue} setValue={(val) => setCurrentValue(val)} />
        </View>
      </View>
